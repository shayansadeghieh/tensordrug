...
def cached(func, debug=False):
    ...
    def wrapper(forward, self, *args, **kwargs):
        ...
        def equal(x, y):
            if isinstance(x, tf.nn):
                x = x.data
            if isinstance(y, tf.nn):
                y = y.data
            if type(x) != type(y):
                return False
            if isinstance(x, tf.Tensor):
                ...
                if x.num_node != y.num_node or x.num_edge != y.num_edge or x.num_relation != y.num_relation:
                    return False
                edge_feature = getattr(x, "edge_feature", tf.Tensor(0, device=x.device))
                y_edge_feature = getattr(y, "edge_feature", tf.Tensor(0, device=y.device))
        ...
        for k, v in arguments.items():
            if isinstance(v, tf.Tensor) or isinstance(v, data.Graph):
                v = v.detach()
