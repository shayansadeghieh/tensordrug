...
class Graph(_MetaContainer):
    ...
    Example::
    >>> graph = data.Graph(tf.random.uniform(10, (30, 2)))
    >>> with graph.node():
    >>>     graph.my_node_attr = tf.random.uniform(10, 5, 5)
    Parameters:
    ...
    def _build_inverted_index(self, mask):
        ...
        ranges = tf.stack([starts, ends], dim=-1)
    ...
    def size(self, dim=None):
        if self.num_relation:
            size = tf.size((self.num_node, self.num_node, self.num_relation))
        else:
            size = tf.size((self.num_node, self.num_node))
        if dim is None:
            return size
        return size[dim]
...
class PackedGraph(Graph):
    ...
    def repeat(self, count):
        """
        Repeat this packed graph. This function behaves similarly to `tf.Tensor.repeat`_.
        .. _torch.Tensor.repeat:
            https://pytorch.org/docs/stable/generated/tf.Tensor.repeat.html
        Parameters:
            count (int): number of repetitions
        Returns:
            PackedGraph
    ...
    def repeat_interleave(self, repeats):
        ...
        .. _torch.repeat_interleave:
            https://pytorch.org/docs/stable/generated/tf.repeat.html
        Parameters:
    ...
    def graph_mask(self, index, compact=False):
        ...
        if compact:
            ...
            num_nodes = tf.zeros_like(self.num_nodes)
        ...
        if compact:
            ...
            num_edges = tf.zeros_like(self.num_edges)
...
def cat(graphs):
    ...
    edge_list = tf.concat([graph.edge_list for graph in graphs])
    pack_num_nodes = tf.stack([graph.num_node for graph in graphs])
    ...
    edge_list[:, :2] += pack_offsets.unsqueeze(-1)
    offsets = tf.concat([graph._offsets for graph in graphs]) + pack_offsets

    edge_weight = tf.concat([graph.edge_weight for graph in graphs])
    num_nodes = tf.concat([graph.num_nodes for graph in graphs])
    num_edges = tf.concat([graph.num_edges for graph in graphs])
