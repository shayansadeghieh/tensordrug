...
class PerfectHash(object):
    ...
    prime = 1000000000039
    max_attempt = 10
    max_input_dim = ((tf.dtypes.int64).max - prime) / prime
    ...
    def __init__(self, keys, weight=None, bias=None, sub_weights=None, sub_biases=None):
        ...
        if weight is None:
            weight = tf.random.uniform(0, self.prime, (1, input_dim), device=keys.device)
        if bias is None:
            bias = tf.random.uniform(0, self.prime, (1,), device=keys.device)
        if sub_weights is None:
            sub_weights = tf.random.uniform(0, self.prime, (num_input, input_dim), device=keys.device)
        if sub_biases is None:
            sub_biases = tf.random.uniform(0, self.prime, (num_input,), device=keys.device)
    ...
    def _construct_hash_table(self):
        ...
        self.table = -tf.ones(self.num_sub_output, dtype=tf.dtypes.int64, device=self.device)
        self.table[sub_index] = tf.range(self.num_input, device=self.device)
    ...
    def __call__(self, keys):
        ...
        """
        keys = tf.convert_to_tensor(keys, dtype=tf.dtypes.int64, device=self.device)
        ...
        final_index[found] = tf.where(equal, found_index, -tf.ones_like(found_index))
        return final_index

    def _reset_hash(self):
        self.weight = tf.random.uniform(self.weight, 0, self.prime)
        self.bias = tf.random.uniform(self.bias, 0, self.prime)

    def _reset_sub_hash(self, mask=None):
        if mask is None:
            self.sub_weights = tf.random.uniform(self.sub_weights, 0, self.prime)
            self.sub_biases = tf.random.uniform(self.sub_biases, 0, self.prime)
        else:
            self.sub_weights[mask] = tf.random.uniform(self.sub_weights[mask], 0, self.prime)
            self.sub_biases[mask] = tf.random.uniform(self.sub_biases[mask], 0, self.prime)
...
class Dictionary(object):
    ...
    Example::
        ...
        >>> keys = tf.Tensor([[0, 0], [1, 1], [2, 2]])
        >>> values = tf.Tensor([[0, 1], [1, 2], [2, 3]])
        >>> d = data.Dictionary(keys, values)
        >>> assert (d[[[0, 0], [2, 2]]] == values[[0, 2]]).all()
        >>> assert (d.has_key([[0, 1], [1, 2]]) == tf.Tensor([False, False])).all()
    ...
    def get(self, keys, default=None):
        ...
        if default is None:
            default = 0
        default = tf.convert_to_tensor(default, dtype=self.values.dtype, device=self.device)
        index = self.hash(keys)
        shape = list(index.shape) + list(self.values.shape[1:])
        values = tf.ones(shape, dtype=self.values.dtype, device=self.device) * default
        found = index != -1
